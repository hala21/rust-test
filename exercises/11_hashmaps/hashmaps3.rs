...
fn build_scores_table(results: &str) -> HashMap<&str, Team> {
    ...
    for line in results.lines() {
        ...
        scores.entry(team_1_name)
            .and_modify(|s: &mut Team|  s.goals_scored += team_1_score)
            .and_modify(|f: &mut Team| f.goals_conceded += team_2_score)
            .or_insert(Team { goals_scored: team_1_score, goals_conceded: team_2_score });

        scores.entry(team_2_name)
            .and_modify(|s: &mut Team|  s.goals_scored += team_2_score)
            .and_modify(|f: &mut Team|  f.goals_conceded += team_1_score)
